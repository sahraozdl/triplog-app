...
import {
  TravelLog, AccommodationLog, AdditionalLog, WorkTimeLog, DailyLogFormState
...
// Type Helpers
type FormState<T> = Omit<T, "_id" | "userId" | "tripId" | "createdAt" | "updatedAt" | "files" | "sealed" | "isGroupSource" | "appliedTo" | "dateTime" | "itemType">;
...
export default function EditDailyLogPage() {
    ...
    // --- GLOBAL STATES ---
    // Stores simple YYYY-MM-DD string for input
    ...
    // --- FORM STATES ---
    const [travel, setTravel] = useState<TravelFormState>(getInitialState("travel"));
    const [workTime, setWorkTime] = useState<WorkTimeFormState>(getInitialState("worktime"));
    const [accommodationMeals, setAccommodationMeals] = useState<AccommodationFormState>(getInitialState("accommodation"));
    const [additional, setAdditional] = useState<AdditionalFormState>(getInitialState("additional"));
    const [logIds, setLogIds] = useState<{ travel?: string; worktime?: string; accommodation?: string; additional?: string; }>({});
    ...
    useEffect(() => {
        ...
        async function fetchAndFillLogs() {
            ...
            try {
                ...
                const logDate = initialLog.dateTime ? initialLog.dateTime.split('T')[0] : ""; 
                
                setTripId(tripIdToFetch);

                // Fetch all logs for this day/user/trip
                const groupRes = await fetch(`/api/daily-logs?tripId=${tripIdToFetch}&userId=${logUserId}&date=${logDate}`);
                ...
                logs.forEach(log => {
                    ...
                    else if (type === "worktime") setWorkTime(formPayload as WorkTimeFormState);
                    else if (type === "accommodation") setAccommodationMeals(formPayload as AccommodationFormState);
                    else if (type === "additional") setAdditional(formPayload as AdditionalFormState);
                });
                
                setLogIds(newLogIds); 
                // Set initial date string (YYYY-MM-DD)
    ...
    async function handleUpdateLog(e: React.FormEvent) {
        ...
        if (!selectedDate) return alert("Date is required.");
        if (!loggedInUserId || !tripId) return alert("User or Trip context missing.");

        setIsSaving(true);
        
        // --- DATE HANDLING ---
        // Convert selected YYYY-MM-DD to ISO String (UTC Noon)
        ...
        const forms = [
            ...
            { type: "accommodation" as const, data: accommodationMeals, id: logIds.accommodation },
            { type: "additional" as const, data: additional, id: logIds.additional },
        ];
        
        forms.forEach(form => {
             // Check if form has data
            const hasData = Object.values(form.data).some(val => val && val !== "" && val !== 0 && val !== false);

            if (form.id) {
                 // UPDATE existing log (PUT)
    ...
    if (loadingLogs) return <div className="p-6 text-center text-lg"><Loader2 className="h-6 w-6 animate-spin mx-auto text-primary mb-2" /> Loading logs...</div>;
    if (!tripId) return <div className="p-6 text-center text-lg text-red-500"><ArrowLeft className="h-4 w-4 inline mr-2"/> Trip context missing.</div>;
        ...
        <div className="w-full flex justify-center px-4 py-8 bg-background min-h-screen">
          <div className="w-full max-w-4xl space-y-6">
            ...
            <div className="flex justify-between items-center pb-4 border-b border-border">
                ...
                <Button variant="ghost" onClick={() => router.push(`/tripDetail/${tripId}`)}>
            ...
            <div className="bg-card p-6 rounded-xl border border-border shadow-sm space-y-4">
                <div className="max-w-sm w-full relative">
                    <Label htmlFor="logDate" className="mb-2 block font-semibold text-foreground">
                        Date (Original: {new Date(originalLogs[0]?.dateTime || Date.now()).toLocaleDateString()})
                    </Label>
            ...
            {/* FORMS */}
            <form id="dailyLogForm" onSubmit={handleUpdateLog} className="flex flex-col gap-6">
              ...
              <Button type="submit" disabled={isSaving} className="mt-6 h-12 text-base">
